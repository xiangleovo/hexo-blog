...
// 获取下一个除夕日期
function getNextChineseNewYearEve() {
    const now = new Date();
    const currentYear = now.getFullYear();
    
    // 已知的除夕日期（可以根据需要扩展）
    const knownDates = {
        2025: new Date("2025-01-28"),
        2026: new Date("2026-02-16"),
        2027: new Date("2027-02-05"),
        2028: new Date("2028-01-25"),
        2029: new Date("2029-02-12"),
        2030: new Date("2030-02-02")
    };
    
    // 检查当前年份的除夕是否已过
    if (knownDates[currentYear] && knownDates[currentYear] > now) {
        return knownDates[currentYear];
    }
    
    // 否则返回下一年的除夕
    const nextYear = currentYear + 1;
    return knownDates[nextYear] || new Date(nextYear, 1, 15); // 默认2月15日
}
...
function cardTimes() {
  ...
  if (e) {
    ...
    // 修改这里：使用动态计算的除夕日期
    const newYearDate = getNextChineseNewYearEve();
    ...
    if (scheduleDateElement) {
        const formattedDate = newYearDate.getFullYear() + "-" + 
            (newYearDate.getMonth() + 1).toString().padStart(2, '0') + "-" + 
            newYearDate.getDate().toString().padStart(2, '0');
